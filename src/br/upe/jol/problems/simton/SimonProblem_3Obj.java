 ...
 */
public class SimonProblem_3Obj extends Problem<Integer> {
	...
	public SimonProblem_3Obj(int numberOfNodes_ppr, int numberOfObjectives) {
	...
	private synchronized void evaluateNetworkEnergyExpenditure(SolutionONTD solution,
			List<Integer> networkRepresentation_ppr,
			Vector<Vector<Double>> adjacencyMatrixLabels,
			Vector<Vector<Double>> adjacencyMatrix) {
		// Gastos de energia = C1*W*Nenlaces + C2*Label_amplificador*4*N_enlaces + C3*nNos*w
		...
		solution.setObjective(2, C1*w*nEnlaces + C2*amplifierEnergyExp_loc*4*nEnlaces + C3*nNodes*w*sumNodeDegree);
	}
...
//				adjacencyMatrix.get(node2).set(node1, 1.0);
	...
	private static final int TIMEOUT_LOC = 3;
	private synchronized void evaluateNetwokBlockingProbability(SolutionONTD solution,
		...
		// numberOfCalls_loc = 1;
		while ((lastSimulation_loc * numberOfCalls_loc < 100) && (i <= TIMEOUT_LOC)) {
			numberOfCalls_loc *= 100;
...
//			System.out.println(pb[i] + " " + cost[i]);
	...
	public static void setUsarCache(boolean usarCache) {
		SimonProblem_3Obj.usarCache = usarCache;
	}
