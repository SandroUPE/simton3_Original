 ...
 * Arquivo: SimtonProblemNonUniformHub.java
 * ****************************************************************************
...
import br.upe.jol.problems.simon.rwa.OpticalNetworkSimulatorAbstract;
 ...
 * @since 02/01/2014
 */
public class SimtonProblemNonUniformHub extends Problem<Integer> {
	...
	public SimtonProblemNonUniformHub(int numberOfNodes_ppr, int numberOfObjectives) {
		super(((numberOfNodes_ppr * numberOfNodes_ppr - numberOfNodes_ppr) / 2) + 2, numberOfObjectives,
				"Projeto de Redes Ópticas");
		redeNeural = new RedeNeural(12, 1, 12, true);
		redeNeural.carregarRede("mlp_12_booster_0_12.txt");
		...
		profile = new NetworkProfile(null, nodes, 200 * 0.01, 0.01, 100000, SNR_BLOCK, true, true, true, true, false,
	...
	public synchronized void evaluate(Solution<Integer> solution) {
		evaluate(solution, this.simulate);
	}

	public synchronized void evaluate(Solution<Integer> solution, boolean simulate) {
		avals++;
		...
		try {
			...
			double[] resultRnas = netwokSimulationRegression((SolutionONTD) solution, cn, lastSimulation);
			double rna = resultRnas[0];
			if (!simulate) {
			...
			sbErrosRna.append(format.format(rna)).append(" ").append(format.format(lastSimulation)).append(" ")
					.append(format.format(Math.pow(lastSimulation - rna, 2))).append("\n");
			System.out.println(format.format(rna) + " " + format.format(lastSimulation) + " "
					+ format.format(Math.pow(lastSimulation - rna, 2)));
			try {
	...
	public synchronized void evaluateNetworkCost(SolutionONTD solution, List<Integer> networkRepresentation_ppr,
	...
	private synchronized void evaluateNetwokBlockingProbability(SolutionONTD solution,
		...
		for (int i = 0; i < maximum; i++) {
			...
			// System.out.printf("Avaliação %d: %.6f\n", i, parcialSimul);
		}
		lastSimulation_loc /= maximum;

		solution.setObjective(0, lastSimulation_loc);
		// }
		// System.out.printf("RNA    = %.8f; SIMTON = %.8f; EMQ = %.8f; Outlier = %s \n",
		// rna, lastSimulation_loc,
		// Math.pow(rna - lastSimulation_loc, 2), outlier ? "Sim" : "Não");
		// totalEvaluations++;

		// System.out.printf("EVALUATIONS (SIMTON, TOTAL, SIMTON/TOTAL) = (%d, %d, %.8f)\n",
		// simtonEvaluations,
		// totalEvaluations, (simtonEvaluations * 100.0) / totalEvaluations);
		// System.out.printf("%.8f\n", lastSimulation_loc);

		// System.out.printf("%.8f %.8f %.8f\n", lastSimulation_loc, rna,
		// Math.abs(lastSimulation_loc - rna) / rna);
		// if (simulate) {
		// solution.setObjective(0, lastSimulation_loc);
		// } else {
		// solution.setObjective(0, rna);
	...
	public double netwokSimulation(double numberOfCalls_loc, SolutionONTD solution,
		...
		profile.cleanBp();
		// C_CallSchedulerNonUniform Scheduler(numOfNodInNet_loc,
		// meanRateBetweenCalls_par/meanRateOfCallsDuration_par)
		simulator.simulate(profile, new CallSchedulerNonUniformHub(14, profile.getMeanRateBetweenCalls()
				/ OpticalNetworkSimulatorAbstract.MEAN_RATE_CALLS_DUR));
		// Uniform(profile.getMeanRateBetweenCalls(),
		// OpticalNetworkSimulatorAbstract.MEAN_RATE_CALLS_DUR,
		// profile.getNodes().size())
	...
	public double[] netwokSimulationRegression(SolutionONTD solution, ComplexNetwork cn, double pbReal) {
		...
		for (int i = 0; i < 14; i++) {
			for (int j = i + 1; j < 14; j++) {
				orderedCloseness.add(linkCloseness[i][j].intValue()
						* AMPLIFIERS_COSTS_AND_LABELS[2][solution.getDecisionVariables()[counter]]);
				counter++;
			}
		}
		...
		double[] input = new double[] {
				...
				cn.getMetricValues().get(TMetric.DENSITY), cn.getMetricValues().get(TMetric.NATURAL_CONNECTIVITY),
				cn.getMetricValues().get(TMetric.PHYSICAL_AVERAGE_PATH_LENGTH),
				cn.getMetricValues().get(TMetric.DFT_LAPLACIAN_ENTROPY), 0, 0, 0, 0, 0, 0 };

		for (int i = 0; i < numLC; i++) {
			input[input.length - (i + 1)] = orderedCloseness.get(orderedCloseness.size() - (i + 1));
		}

		// String hash = am.toString() + input[0] + input[1];
		// if (!setNets.contains(hash)) {
		// evals++;
		// sbNetworks.append(getLine(this, solution,
		// linkCloseness)).append("\n");
		//
		// if (evals % INTERVAL_SAVE == 0) {
		// seq++;
		// try {
		// FileWriter fw = new FileWriter(new File("complenet_consol_" + seq +
		// ".txt"));
		//
		...
		// setNets.add(hash);
		// }
		return new double[] { redeNeural.obterSaida(input, true), 0 };
	}
