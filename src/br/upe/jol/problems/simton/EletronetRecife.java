 ...
 * @since 23/03/2014
 */
public class EletronetRecife extends Problem<Integer> {
	...
	public static double[][] NODES_POSITIONS = new double[][] { { -34.876471 * 110, -8.067570 * 110 },
			{ -34.903493 * 110, -8.127529 * 110 }, { -34.897029 * 110, -8.030329 * 110 },
			{ -34.845874 * 110, -7.989023 * 110 }, { -34.937690 * 110, -8.008016 * 110 },
			{ -34.938720 * 110, -8.065128 * 110 }, { -35.120337 * 110, -8.113735 * 110 } };
	...
	public EletronetRecife(int numberOfNodes_ppr, int numberOfObjectives) {
		...
		redeNeural.carregarRede("mlp_7_booster_23_15.txt");
		...
		profile = new NetworkProfile(null, nodes, 55 * 0.01, 0.01, 100000, SNR_BLOCK, true, true, true, true, false,
	...
	public synchronized void evaluate(Solution<Integer> solution, boolean simulate) {
		...
		try {
			if (cn.getMetricValues().get(TMetric.ALGEBRAIC_CONNECTIVITY) <= 1e-3) {
...
//			if (!simulate && cn.getMetricValues().get(TMetric.ALGEBRAIC_CONNECTIVITY) > 1e-3) {
...
//			}
			...
			// .append(format.format(Math.pow(lastSimulation - rna,
			// 2))).append("\n");
			// System.out.println(format.format(rna) + " " +
			// format.format(lastSimulation) + " "
	...
	public double[] netwokSimulationRegression(SolutionONTD solution, ComplexNetwork cn, double pbReal) {
		...
		// cn.getMetricValues().get(TMetric.ALGEBRAIC_CONNECTIVITY) > 1e-4) {
		// evals++;
		...
		// setNets.add(hash);
		// }
		System.out.println("PB RNA = " + redeNeural.obterSaida(input, true));
		return new double[] { redeNeural.obterSaida(input, true), 0 };

	}

	public static void main(String[] args) {
		revalidadeFile(new String[] { "C:/BACKUP_PENDRIVE_AZUL/Mestrado/Experimentos/2o/spea2/perfil4/r01/_spea2_50_100_100_001_1000_var.txt" });
	}
