 ...
 * @since 13/06/2015
 */
public class EaCsmNU extends Problem<Integer> {
	...
	//
	private int[] limitsMinCalls = new int[] { 500, 1000 };

	// private double[] limits = new double[] { 0.01, 0.001, 0.0001, 0.00001 };

	// private int[] limitsMinCalls = new int[] { 0, 500, 250, 250 };

	private double[] limitsMaxCalls = new double[] { 1000, 1000000 };

	// private double[] limitsMaxCalls = new double[] { 0, 10000, 100000,
	// 1000000 };
	...
	public EaCsmNU(int numberOfNodes_ppr, int numberOfObjectives, String gmlFile, double networkLoad) {
		this(numberOfNodes_ppr, numberOfObjectives, dao.loadGmlData(gmlFile), networkLoad);
	}

	public EaCsmNU(int numberOfNodes_ppr, int numberOfObjectives, GmlData data, double networkLoad) {
	...
	public synchronized void evaluate(Solution<Integer> solution) {
		...
		try {
			...
			if (cn.getMetricValues().get(TMetric.ALGEBRAIC_CONNECTIVITY) <= 1e-5) {
				...
				minCalls = limitsMinCalls[0];
				maxCalls = limitsMaxCalls[0];
				double pb = netwokSimulation(maxCalls, (SolutionONTD) solution, networkRepresentation_ppr,
