 ...
 * Arquivo: SurrogateSimtonBasedNonUniform.java
 * ****************************************************************************
 * Histórico de revisões
 * Nome				Data		Descrição
 * ****************************************************************************
 * Danilo Araújo	24/09/2014		Versão inicial
 * ****************************************************************************
 */
 ...
 * @since 24/09/2014
 */
public class SurrogateSimtonBasedNonUniform extends Problem<Integer> {
	...
	private static final double NC = 400;

	private double[] limits = new double[] { 0.10, 0.01, 0.001, 0.0001, 0.00001 };

//	 private double[] limits = new double[] { 0.10, 0.00001 };

	private int[] limitsMinCalls = new int[] { 0, 1000, 1000, 1000, 1000 };

//	 private int[] limitsMinCalls = new int[] { 0, 500 };

	private double[] limitsMaxCalls = new double[] { 0, 2000, 10000, 1000000, 1000000 };

//	 private double[] limitsMaxCalls = new double[] { 0, 1000000 };

	public SurrogateSimtonBasedNonUniform(int numberOfNodes_ppr, int numberOfObjectives) {
		this(numberOfNodes_ppr, numberOfObjectives, 200);
	}

	public SurrogateSimtonBasedNonUniform(int numberOfNodes_ppr, int numberOfObjectives, int networkLoad) {
		...
		redeNeural.carregarRede("mlp_7_traffic_0_15-non-uniform.txt");
	...
	private Map<Interval, StatsSummary> getMap(Problem<Integer> problem) {
		...
		double increment = (problem.getUpperLimitObjective()[1] - problem.getLowerLimitObjective()[1]) / NC;
	...
	private void updateMapCosts(Map<Interval, StatsSummary> map, double pb, double cost, boolean r, double iteration) {
		double mean = 0;
		double stdDeviation = 0;
		StatsSummary stats = null;
		double increment = (this.getUpperLimitObjective()[1] - this.getLowerLimitObjective()[1]) / NC;
		double incrPb = 0.1;
		for (Interval interval : map.keySet()) {
			...
			if (interval.contains(cost)) {
				...
				System.out.printf("intervalo = [%s]; média = %.4f; desvio = %.4f \n", interval.toString(), mean,
						stdDeviation);
				stats = new StatsSummary(mean, stdDeviation, iteration);
				map.put(interval, stats);

				// verificar se os adjacentes estão vazios e preencher
				if (r) {
					if (assertSolutionEvaluation(pb, cost + increment, iteration) == NOT_INITIALIZED) {
						updateMapCosts(map, pb * (1 - incrPb) > .00001 ? pb * (1 - incrPb) : 00001, cost + increment, false, iteration);
					}
					if (assertSolutionEvaluation(pb, cost - increment, iteration) == NOT_INITIALIZED) {
						updateMapCosts(map, pb * (1 + incrPb) < 1 ? pb * (1 + incrPb) : 1, cost - increment, false, iteration);
					}	
				}
	...
	private void updateMapCosts(Map<Interval, StatsSummary> map, Solution<Integer> sol, double iteration) {
		updateMapCosts(map, sol.getObjective(0), sol.getObjective(1), true, iteration);
	}

	private int assertSolutionEvaluation(SolutionONTD sol, double iteration) {
		double pb = sol.getObjective(0);
		double cost = sol.getObjective(1);

		return assertSolutionEvaluation(pb, cost, iteration);
	}

	private int assertSolutionEvaluation(double pb, double cost, double iteration) {
		StatsSummary stats = null;
		for (Interval interval : mapCosts.keySet()) {
			if (interval.contains(cost)) {
				stats = mapCosts.get(interval);
				if (iteration - stats.getLastUpdate() > 5 || (stats.getMean() == 0 && stats.getStdDeviation() == 0)) {
	...
	@Override
	public synchronized void evaluate(Solution<Integer> solution, double iteration) {
		...
		try {
			...
			if (cn.getMetricValues().get(TMetric.ALGEBRAIC_CONNECTIVITY) <= 1e-5) {
				...
				if (pb < limits[i]) {
					...
					solution.setObjective(0, pb);
					while (pb < limits[i]
							&& (assertSolutionEvaluation((SolutionONTD) solution, iteration) == NOT_INITIALIZED || assertSolutionEvaluation((SolutionONTD) solution, iteration) == OUTSIDE_RANGE_LEFT)
							&& i < limits.length - 1) {
						i++;
						minCalls = limitsMinCalls[i];
						maxCalls = limitsMaxCalls[i];
				...
				if (pb >= limits[i] && assertSolutionEvaluation((SolutionONTD) solution, iteration) != OUTSIDE_RANGE_RIGHT) {
					updateMapCosts(mapCosts, solution, iteration);
				}
