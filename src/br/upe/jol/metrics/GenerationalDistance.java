...
public class GenerationalDistance {
	...
	public double generationalDistance(double[][] front, double[][] trueParetoFront, int numberOfObjectives) {
		...
		maximumValue = Util.getMaximumValues(trueParetoFront, numberOfObjectives);
		minimumValue = Util.getMinimumValues(trueParetoFront, numberOfObjectives);

		// STEP 2. Get the normalized front and true Pareto fronts
		normalizedFront = Util.getNormalizedFront(front, maximumValue, minimumValue);
		normalizedParetoFront = Util.getNormalizedFront(trueParetoFront, maximumValue, minimumValue);

		// STEP 3. Sum the distances between each point of the front and the
		// nearest point in the true Pareto front
		double sum = 0.0;
		for (int i = 0; i < front.length; i++)
			sum += Math.pow(Util.distanceToClosedPoint(normalizedFront[i], normalizedParetoFront), pow_);
