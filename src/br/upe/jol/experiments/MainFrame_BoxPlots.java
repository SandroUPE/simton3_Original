...
import br.upe.jol.base.Metricas;
 ...
 * 
 */
public class MainFrame_BoxPlots implements Runnable{
	...
	private Metricas<Double>[] metricas;
	...
	public synchronized void createChart(ChartViewer viewer, Observer[] observers) {
		...
		for (Observer observer : observers){
			...
			atualizacao = true;
			int size = observer.getSolutionSet().getSolutionsList().size();
			double[] dataX0 = new double[size];
			double[] dataY0 = new double[size];
			
			size = this.updateSize(size, observer.getSolutionSet().size());
			System.out.println(" ");
			System.out.println(size);
			try{
				for (int i = 0; i < size; i++){
					SolutionSet<Double> solutionSet = observer.getSolutionSet();
					dataX0[i] = solutionSet.get(i).getObjective(0);
					dataY0[i] = solutionSet.get(i).getObjective(1);
					size = this.updateSize(size, observer.getSolutionSet().size());
					System.out.println(size);
				}
			}catch(IndexOutOfBoundsException e){
				System.out.println("Size = "+ size);
				System.out.println("Tamanho Real =" + observer.getSolutionSet().size());
			}
	...
	private synchronized int updateSize(int sizeOld, int sizeNow){
		if(sizeOld > sizeNow){
			return sizeNow;
		}
	...
	public void updateBoxWhiskerChart(ChartViewer viewer){
		if(metricas == null){
			return;
		}
		
		StatisticsUtil util = StatisticsUtil.getInstance();
        String[] labels = new String[4];        
		double[] min = new double[metricas.length];
        double[] Q1 = new double[metricas.length];
        double[] Q2 = new double[metricas.length];
        double[] Q3 = new double[metricas.length];
        double[] max = new double[metricas.length];

        
        for (int i = 0; i < metricas.length; i++){
        	labels[i] = metricas[i].gettAlgorithm().toString();
        	double[] aux = null;
        	
        	if(mList.getSelectedValue().toString().equalsIgnoreCase(TMetrics.HIPERVOLUME.toString())){
        		aux = metricas[i].getHyperVolumes();
        	}else if(mList.getSelectedValue().toString().equalsIgnoreCase(TMetrics.SPACING.toString())){
        		aux = metricas[i].getSpacings();
        	}else if(mList.getSelectedValue().toString().equalsIgnoreCase(TMetrics.SPREAD.toString())){
        		aux = metricas[i].getMaximumSpreads();
        	}
        	
        	if(aux.length > 0){
	        	Arrays.sort(aux);
	        	double[] quartis = util.getQuartis(aux);
	        	min[i] = aux[0];
	        	Q1[i] = quartis[0];
	        	Q2[i] = quartis[1];
	        	Q3[i] = quartis[2];
	        	max[i] = aux[aux.length-1];
        	}
        }
        ...
        c.addBoxWhiskerLayer(Q1, Q3, min, max, Q2, 0x9999ff,
	...
	private JPanel getPanelRuns(){
		...
		final MainFrame_BoxPlots mf = this;
		...
		mList.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent arg0) {
				updateBoxWhiskerChart(boxPlotViewer);
			}
		});
	...
	private JPanel getPanelConfigs() throws ParseException{
		...
		panelConfigNSGAII.add(new JLabel("<html>N&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Tx Mutaï¿½ï¿½o&nbsp;&nbsp;&nbsp;Tx Crossover</html>"));
		...
		panelConfigSpea2.add(new JLabel("<html>N&nbsp;&nbsp;&nbsp;F&nbsp;&nbsp;Tx Mutaï¿½ï¿½o&nbsp;&nbsp;Tx Crossover</html>"));
		...
		//panelConfigPesaII.add(new JLabel("<html>N&nbsp;&nbsp;&nbsp;F&nbsp;&nbsp;Tx Mutaï¿½ï¿½o&nbsp;&nbsp;Tx Crossover</html>"));
...
//		panelEsq.add(paneldnMoPso);
	...
	public void executarAlgoritmos(){
		...
		metricas = new Metricas[tAlgorithms.length];
		...
		for (int i = 0; i < tAlgorithms.length; i++){
			...
			metricas[i] = new Metricas<Double>(this.configTO.getnRuns());
			metricas[i].settAlgorithm(tAlgorithm);
		...
		for (int i = 0; i < tAlgorithms.length; i++){
			...
			final Metricas<Double> m = metricas[i];
			Thread thread = new Thread(){
				public void run(){
					int cont = configTO.getnRuns();
					while(cont>0){
						SolutionSet<Double> solutionSet = a.execute();
						o.setSolutionSet(solutionSet);
						m.calcularMetricas(solutionSet);
						updateBoxWhiskerChart(boxPlotViewer);
						cont--;
					}
				}
	...
	public static void main(String[] args) {
		MainFrame_BoxPlots app = new MainFrame_BoxPlots();
		try {
			app.initialize();
		} catch (Exception e) {
			e.printStackTrace();
